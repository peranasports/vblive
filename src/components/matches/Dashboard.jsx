...
export default function Dashboard({ match, selectedGame, selectedTeam }) {
  const [categoryLeaders, setCategoryLeaders] = useState(null)

  const sortCategoryLeaders = () => {
    var catleaders = []
    if (match === undefined || match == null) {
      return
    }
    var sis = selectedGame === 0 ? selectedTeam === 0 ? match.teamA.statsItems : match.teamB.statsItems :
      selectedTeam === 0 ? match.sets[selectedGame - 1].teamAStatsItems : match.sets[selectedGame - 1].teamBStatsItems
    ...
    passers.players = []
    for (var np = 0; np < bestPassers.length; np++) {
    ...
    hitters.players = []
    for (var np = 0; np < bestHitters.length; np++) {
      var si = bestHitters[np]
      if (si.player == null) {
    ...
    servers.players = []
    for (var np = 0; np < bestServers.length; np++) {
      var si = bestServers[np]
      if (si.player == null) {
      ...
      <div className="stats stats-vertical shadow mt-4">
        ...
        <div className="stat">
          <div className="flex stat-figure text-lime-500 justify-between">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="inline-block w-8 h-8 stroke-current mb-2">
