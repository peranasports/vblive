...
export function createStatsItem(player, set) {
  ...
  si.gamesPlayed = 0;
  si.PointsWon = 0;
  si.SideOuts = [0, 0, 0, 0, 0, 0, 0];
  si.BreakPoints = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBalls = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFails = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallKills = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutOn2s = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallSpikes = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutOppErrors = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutPercent = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallPercent = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutTransitionPercent = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutOppErrorPercent = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallKillPercent = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallOppServeError = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutFirstBallOppErrors = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutKills = [0, 0, 0, 0, 0, 0, 0];
  si.SideOutOppErrors = [0, 0, 0, 0, 0, 0, 0];
  return si;
}
...
export function addStatsItem(ssSource, ssDestination) {
  ...
  if (ssSource.SideOuts) {
    for (var i = 0; i < 7; i++) {
      ssDestination.SideOuts[i] += ssSource.SideOuts[i];
      ssDestination.BreakPoints[i] += ssSource.BreakPoints[i];
      ssDestination.SideOutFirstBalls[i] += ssSource.SideOutFirstBalls[i];
      ssDestination.SideOutFails[i] += ssSource.SideOutFails[i];
      ssDestination.SideOutFirstBallKills[i] +=
        ssSource.SideOutFirstBallKills[i];
      ssDestination.SideOutOn2s[i] += ssSource.SideOutOn2s[i];
...
export function calculateAllStats(si) {
  ...
  si.PointsNett =
    ...
    si.Blck3 +
    si.Blck4 / 2.0 -
    (si.Spike0 + si.Serve0 + si.Blck0 + si.BHE + si.Pass0 + si.Set0);
  ...
  si.ServeAverageSpeed =
    si.ServeTotal != 0 ? si.ServeTotalSpeed / si.ServeTotal : 0;

  si.SpikeEfficiencyString =
    si.SpikeEfficiency === -3
      ? ""
      : (si.SpikeEfficiency * 100).toFixed(0) + "%";
  ...
  si.BlocksPerGame =
    si.gamesPlayed == 0 ? "" : (si.BlockPoints / si.gamesPlayed).toFixed(2);

  si.DigsPerGame =
    si.gamesPlayed == 0 ? "" : (si.Dig / si.gamesPlayed).toFixed(2);
  si.AcesPerGame =
    si.gamesPlayed == 0 ? "" : (si.Serve3 / si.gamesPlayed).toFixed(2);
  si.BlocksPerGame =
    si.gamesPlayed == 0 ? "" : (si.BlockPoints / si.gamesPlayed).toFixed(2);
  si.PointsPerGame =
    si.gamesPlayed == 0 ? "" : (si.PointsWon / si.gamesPlayed).toFixed(2);
  si.KillsPerGame =
    si.gamesPlayed == 0 ? "" : (si.Spike3 / si.gamesPlayed).toFixed(2);

  for (var i = 0; i < 7; i++) {
    var tot = si.SideOutFails[i] + si.SideOuts[i];
    si.SideOutPercent[i] = !tot || tot == 0 ? 0 : (si.SideOuts[i] * 100) / tot;
    ...
    si.SideOutFirstBallKillPercent[i] =
      si.SideOutFirstBallSpikes[i] == 0
        ? 0
        : (si.SideOutFirstBallKills[i] * 100) / si.SideOutFirstBallsSpikes[i];
  }
  si.pointPercent =
    si.ServeTotal == 0 ? 0 : (si.BreakPoints[0] * 100) / si.ServeTotal;
  ...
  si.GoodServePercent =
    si.ServeTotal == 0 ? 0 : (si.GoodServe * 100) / si.ServeTotal;
...
export function calculateSideoutStats(m, appName) {
  ...
  m.teamA.statsItems[0].pointPercent =
    m.teamA.statsItems[0].ServeTotal == 0
      ? 0
      : (m.teamA.statsItems[0].BreakPoints[0] * 100) /
        m.teamA.statsItems[0].ServeTotal;
  m.teamB.statsItems[0].BreakPoints = m.teamA.statsItems[0].SideOutFails;
  m.teamB.statsItems[0].pointPercent =
    m.teamB.statsItems[0].ServeTotal == 0
      ? 0
      : (m.teamB.statsItems[0].BreakPoints[0] * 100) /
        m.teamB.statsItems[0].ServeTotal;
  for (var nd = 0; nd < m.sets.length; nd++) {
    var game = m.sets[nd];
    game.teamAStatsItems[0].BreakPoints = game.teamBStatsItems[0].SideOutFails;
    game.teamAStatsItems[0].pointPercent =
      game.teamBStatsItems[0].ServeTotal == 0
        ? 0
        : (game.teamAStatsItems[0].BreakPoints[0] * 100) /
          game.teamAStatsItems[0].ServeTotal;
    game.teamBStatsItems[0].BreakPoints = game.teamAStatsItems[0].SideOutFails;
    game.teamBStatsItems[0].pointPercent =
      game.teamBStatsItems[0].ServeTotal == 0
        ? 0
        : (game.teamBStatsItems[0].BreakPoints[0] * 100) /
          game.teamBStatsItems[0].ServeTotal;
  }
  return m;
}
...
export function calculateSideoutStatsForTeam(m, tm, appName) {
  ...
  for (var nd = 0; nd < m.sets.length; nd++) {
    ...
    for (var nr = 1; nr < 7; nr++) {
      ...
      siGame.SideOutFirstBallPercent[nr] =
        setAllPasses[nr] > 0 ? (setFBSideOut[nr] * 100) / setAllPasses[nr] : 0;
      siGame.SideOutPercent[nr] =
        setAllPasses[nr] > 0 ? (setSideOut[nr] * 100) / setAllPasses[nr] : 0;
      siGame.SideOutFailsPercent[nr] = 100 - siGame.SideOutPercent[nr];

      siGame.SideOutFirstBallKills[nr] = setFBKillSideOut[nr];
      siGame.SideOutFirstBallKillPercent[nr] =
        setAllPasses[nr] > 0
          ? (setFBKillSideOut[nr] * 100) / setAllPasses[nr]
          : 0;
      siGame.SideOutOppErrors[nr] = setOppErrorSideOut[nr];
      siGame.SideOutOppErrorPercent[nr] =
        setAllPasses[nr] > 0
          ? (setOppErrorSideOut[nr] * 100) / setAllPasses[nr]
          : 0;
      siGame.SideOutFirstBallOppServeError[nr] = setFBOppServeErrorSideOut[nr];
      siGame.SideOutFirstBallOppServeErrorPercent[nr] =
        setAllPasses[nr] > 0
          ? (setFBOppServeErrorSideOut[nr] * 100) / rotAllPasses[nr]
          : 0;
    ...
    siGame.SideOutFirstBallPercent[0] =
      setAllPasses[0] > 0 ? (setFBSideOut[0] * 100) / setAllPasses[0] : 0;
    siGame.SideOutPercent[0] =
      setAllPasses[0] > 0 ? (setSideOut[0] * 100) / setAllPasses[0] : 0;
    siGame.SideOutFailsPercent[0] =
      setAllPasses[0] > 0 ? 100 - siGame.SideOutPercent[0] : 0;

    siGame.SideOutFirstBallKills[0] = setFBKillSideOut[0];
    siGame.SideOutFirstBallKillPercent[0] =
      setAllPasses[0] > 0 ? (setFBKillSideOut[0] * 100) / setAllPasses[0] : 0;
    siGame.SideOutOppErrors[0] = setOppErrorSideOut[0];
    siGame.SideOutOppErrorPercent[0] =
      setAllPasses[0] > 0 ? (setOppErrorSideOut[0] * 100) / setAllPasses[0] : 0;
    siGame.SideOutFirstBallOppServeError[0] = setFBOppServeErrorSideOut[0];
    siGame.SideOutFirstBallOppServeErrorPercent[0] =
      setAllPasses[0] > 0
        ? (setFBOppServeErrorSideOut[0] * 100) / setAllPasses[0]
        : 0;
  ...
  for (var nr = 1; nr < 7; nr++) {
    ...
    siMatch.SideOutFirstBallPercent[nr] =
      rotAllPasses[nr] > 0 ? (rotFBSideOut[nr] * 100) / rotAllPasses[nr] : 0;
    siMatch.SideOutPercent[nr] =
      rotAllPasses[nr] > 0 ? (rotSideOut[nr] * 100) / rotAllPasses[nr] : 0;
    siMatch.SideOutFailsPercent[nr] =
      rotAllPasses[nr] > 0 ? 100 - siMatch.SideOutPercent[nr] : 0;

    siMatch.SideOutFirstBallKills[nr] = rotFBKillSideOut[nr];
    siMatch.SideOutFirstBallKillPercent[nr] =
      rotAllPasses[nr] > 0
        ? (rotFBKillSideOut[nr] * 100) / rotAllPasses[nr]
        : 0;
    siMatch.SideOutOppErrors[nr] = rotOppErrorSideOut[nr];
    siMatch.SideOutOppErrorPercent[nr] =
      rotAllPasses[nr] > 0
        ? (rotOppErrorSideOut[nr] * 100) / rotAllPasses[nr]
        : 0;
    siMatch.SideOutFirstBallOppServeError[nr] = rotFBOppServeErrorSideOut[nr];
    siMatch.SideOutFirstBallOppServeErrorPercent[nr] =
      rotAllPasses[nr] > 0
        ? (rotFBOppServeErrorSideOut[nr] * 100) / rotAllPasses[nr]
        : 0;
  ...
  siMatch.SideOutFirstBallPercent[0] =
    rotAllPasses[0] > 0 ? (rotFBSideOut[0] * 100) / rotAllPasses[0] : 0;
  siMatch.SideOutPercent[0] =
    rotAllPasses[0] > 0 ? (rotSideOut[0] * 100) / rotAllPasses[0] : 0;
  siMatch.SideOutFailsPercent[0] =
    rotAllPasses[0] > 0 ? 100 - siMatch.SideOutPercent[0] : 0;
  siMatch.SideOutFirstBallKills[0] = rotFBKillSideOut[0];
  siMatch.SideOutFirstBallKillPercent[0] =
    rotAllPasses[0] > 0 ? (rotFBKillSideOut[0] * 100) / rotAllPasses[0] : 0;
  siMatch.SideOutOppErrors[0] = rotOppErrorSideOut[0];
  siMatch.SideOutOppErrorPercent[0] =
    rotAllPasses[0] > 0 ? (rotOppErrorSideOut[0] * 100) / rotAllPasses[0] : 0;
  siMatch.SideOutFirstBallOppServeError[0] = rotFBOppServeErrorSideOut[0];
  siMatch.SideOutFirstBallOppServeErrorPercent[0] =
    rotAllPasses[0] > 0
      ? (rotFBOppServeErrorSideOut[0] * 100) / rotAllPasses[0]
      : 0;
...
export function getDVRalliesInGameForTeam(g, tm) {
  ...
  for (var en = 0; en < events.length; en++) {
    ...
    if (hs1 != hs2 || as1 != as2 || en == events.length - 1) {
      ...
      for (var ne = 0; ne < evs.length; ne++) {
        ...
        if (r.startTime == null) {
          var leadtime =
            ev.videoLeadTime === 0 || ev.videoLeadTime === undefined
              ? 2
              : ev.videoLeadTime;
          try {
            r.startTime =
              ev.TimeStamp !== null && ev.TimeStamp !== undefined
                ? new Date(ev.TimeStamp.getTime() - leadtime * 1000)
                : null;
          } catch (error) {
            console.log(error);
          }
        ...
        var lagtime =
          ev.videoLagTime === 0 || ev.videoLagTime === undefined
            ? 3
            : ev.videoLagTime;
        try {
          r.endTime =
            ev.TimeStamp !== null && ev.TimeStamp !== undefined
              ? new Date(ev.TimeStamp.getTime() + lagtime * 1000)
              : null;
        } catch (error) {
          console.log(error);
        }
      ...
      r.endEvent = laste;
      var lagtime =
        laste.videoLagTime === 0 || laste.videoLagTime === undefined
          ? 3
          : laste.videoLagTime;
      try {
      ...
      var leadtime =
        laste.videoLeadTime === 0 || laste.videoLeadTime === undefined
          ? 2
          : laste.videoLeadTime;
      try {
  ...
  for (var nr = 0; nr < a.length; nr++) {
    ...
    if (
      mr.passing === true &&
      mr.passEvent !== undefined &&
      isPlayerInTeam(mr.passEvent.Player, tm, match)
    ) {
      aa.push(mr);
    }
...
function eventOutcome(e) {
  ...
  if (
    et == kOppositionError ||
    et == kOppositionHitError ||
    et == kOppositionServeError
  ) {
    return 1;
  }
  if (
    et == kOppositionScore ||
    et == kOppositionHitKill ||
    et == kOppositionServeAce
  ) {
    return -1;
  }
  ...
  if (grade == 2 && et == kSkillBlock) {
    // block solo
    return 1;
  }
  if (grade == 3 && et == kSkillBlock) {
    // block assist
    return 2;
  }
...
function findSetter(pls) {
  for (var n = 0; n < pls.length; n++) {
    var pl = pls[n];
    if (
      pl.PositionString !== undefined &&
      pl.PositionString.includes("Setter")
    ) {
      return pl;
    }
  }
...
export function getVBStatsRalliesInGameForTeam(g, tm) {
  ...
  var currentOppositionSetter = getPlayerByGuid(
    g.oppPrimarySetterGuid,
    g.match
  );
    ...
    if (e.EventType == kSubstitution) {
      ...
      if (pl != null) {
        ...
        if (e.Player == currentSetter) {
          ...
          g.match.teamA.players.filter((obj) => obj.Guid === e.Player.Guid)
            .length > 0
        ) {
          if (
            currentSetter == null &&
          ...
          g.match.teamB.players.filter((obj) => obj.Guid === e.Player.Guid)
            .length > 0
        ) {
          if (
            currentOppositionSetter == null &&
            pl.PositionString !== undefined &&
            pl.PositionString.includes("Setter")
          ) {
            currentOppositionSetter = pl;
    ...
    if (hs1 != hs2 || as1 != as2) {
      ...
      for (var ne = 0; ne < evs.length; ne++) {
        ...
        if (skill == kSkillPass && isHomeEvent) {
          ...
          (skill == kSkillServe && isAwayEvent && grade == 3)
        ) {
          r.passing = true;
          r.outcome = 0;
          r.sideoutType = kUnsuccessful;
          ...
          (skill == kSkillServe && isAwayEvent && grade == 0)
        ) {
          r.passing = true;
          r.outcome = 1;
          r.sideout = true;
          ...
          (skill == kSkillSpike && isAwayEvent && grade == 3)
        ) {
          r.outcome = 0;
          if (r.passing) {
            r.sideoutType = kUnsuccessful;
          ...
          (skill == kSkillSpike && isAwayEvent && grade == 0)
        ) {
          r.outcome = 1;
          if (r.passing) {
            r.sideout = true;
